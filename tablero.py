...
class Tablero:
    ...
    def mover_fantasma_rojo(self, fantasma):
        # Rojo: siempre seguir a Pac-Man (o alejar si asustado)
    ...
    def mover_fantasma_rosa(self, fantasma):
        # Rosa: emboscar = mover con modo "posicion" hacia la posición futura de Pac-Man
        if self.victoria or self.pacman.en_muerte:
            return False
        if fantasma.asustado and not fantasma.en_trampa():
            self.movimiento_simple(fantasma, modo="alejar")
    ...
    def mover_fantasma_azul(self, fantasma):
        if self.victoria or self.pacman.en_muerte:
            return False
        if not fantasma.en_trampa() and (time.time() - fantasma.ultimo_cambio_modo >= 10):
    ...
    #---------------------------NUEVO MÉTODO DE MOVIMIENTO SIMPLE------------#
